...
function App() {
  ...
  const content = state.loading ? <Loading /> : state.error ? <Empty icon={CircleAlert} title="Backend belum terhubung" detail={state.error} action={<button className="button" onClick={load}>Coba lagi</button>} /> : {
    ...
    'my-work': <MyWork assignments={data.assignments} tasks={data.tasks} members={data.members} />,
...
function MyWork({ assignments, tasks, members }) { return <div className="page"><PageHeader eyebrow="Assignment overview" title="My work" detail="Lihat assignment yang sudah diputuskan oleh manusia." /><div className="work-list">{assignments.map((assignment) => { const task = tasks.find((item) => item.id === assignment.task); const member = members.find((item) => item.id === assignment.member); return <article className="assignment" key={assignment.id}><span className="avatar small">{initials(member?.name)}</span><div><strong>{task?.title || 'Task'}</strong><p>{member?.name || 'Member'} · {assignment.reason || 'Manual assignment'}</p></div><span className="status status-in-progress">Assigned</span></article>; })}{!assignments.length && <Empty icon={BriefcaseBusiness} title="Belum ada assignment" detail="Assignment akan muncul setelah team lead mengambil keputusan." />}</div></div>; }
